...
class PruebaView(APIView):
    ...
    def post(self, request: Request):
        ...
        # retornará Verdadero o Falso si la data es correcta
...
class CategoriaView(APIView):
    ...
    def get(self, request: Request):
        ...
        # NOTA: cuando se pasa instancias se utiliza el parámetro 'instances' y cuando se pasa información para validar se utiliza el parámetro 'data'
        data_serializada = CategoriaSerializer(instance=categorias, many=True)
        print(categorias)

        return Response(data={
            # convierte las inatancias de la clase en un diccionario
...
class ProductosView(APIView):
    ...
    def get(self, request: Request):
        # Pasos para hacer la paginación manual
        # Primero obtenemos los query params que me servirán para orientarme en la página
        page = int(request.query_params.get('page'))
        perPage = int(request.query_params.get('perPage', 10))
        # cuántos te vas a saltar
        skip = (page - 1) * perPage
        # cuántos vas a tomar, es el mismo valor que PerPage
        take = perPage * page
